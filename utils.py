...
async def load_songs():
    ...
    global songs_cache
    logging.info("🌐 從 R2 載入 songs.json")
    async with aiohttp.ClientSession() as session:
        async with session.get(R2_SONGS_JSON_URL) as resp:
            text = await resp.text()  # 忽略 content-type
...
def get_song_info_by_id(song_id: int):
    if songs_cache is None:
        logging.warning("⚠️ songs_cache 尚未初始化，請先呼叫 await load_songs()")
        return None
...
class GuildState:
    ...
    async def start_playing(self, guild, text_channel, voice_channel):
        ...
        logging.info(f"🔊 正在播放：{song_info['title']} - {song_info['artist']} ({song_info['url']})")
        ...
        try:
            ...
            self.vc.play(
                FFmpegPCMAudio(url),
                after=after_callback_factory(guild, text_channel, self.loop)
            )
...
def get_guild_state(guild, loop=None):
    if guild.id not in guild_states:
        guild_states[guild.id] = GuildState(loop or asyncio.get_event_loop())
...
def after_callback_factory(guild, channel, loop):
    def callback(error):
        try:
